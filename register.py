...
def linux_pythonpath(root):
    ...
    return f'{root}/lib/python{sys.version_info[0]}/dist-packages'
...
def make_kernel_env(args):
    ...
    if args.swift_toolchain is not None:
        ...
        if platform.system() == 'Linux':
            kernel_env['PYTHONPATH'] = linux_pythonpath(f'{args.swift_toolchain}/usr')
            kernel_env['LD_LIBRARY_PATH'] = f'{args.swift_toolchain}/usr/lib/swift/linux'
            kernel_env['REPL_SWIFT_PATH'] = f'{args.swift_toolchain}/usr/bin/repl_swift'
            kernel_env['SWIFT_BUILD_PATH'] = f'{args.swift_toolchain}/usr/bin/swift-build'
            kernel_env[
                'SWIFT_PACKAGE_PATH'
            ] = f'{args.swift_toolchain}/usr/bin/swift-package'
        elif platform.system() == 'Darwin':
            kernel_env[
                'PYTHONPATH'
            ] = f'{args.swift_toolchain}/System/Library/PrivateFrameworks/LLDB.framework/Resources/Python'
            kernel_env['LD_LIBRARY_PATH'] = f'{args.swift_toolchain}/usr/lib/swift/macosx'
            kernel_env[
                'REPL_SWIFT_PATH'
            ] = f'{args.swift_toolchain}/System/Library/PrivateFrameworks/LLDB.framework/Resources/repl_swift'
        ...
        swift_build_dir = f'{args.swift_build}/swift-linux-x86_64'
        lldb_build_dir = f'{args.swift_build}/lldb-linux-x86_64'

        kernel_env['PYTHONPATH'] = linux_pythonpath(lldb_build_dir)
        kernel_env['LD_LIBRARY_PATH'] = f'{swift_build_dir}/lib/swift/linux'
        kernel_env['REPL_SWIFT_PATH'] = f'{lldb_build_dir}/bin/repl_swift'
        ...
        lldb_framework = f'{args.xcode_path}/Contents/SharedFrameworks/LLDB.framework'
        xcode_toolchain = f'{args.xcode_path}/Contents/Developer/Toolchains/XcodeDefault.xctoolchain'
    ...
    if args.swift_python_use_conda:
        if platform.system() == 'Darwin':
            libpython = glob(f'{sys.prefix}/lib/libpython*.dylib')[0]
        elif platform.system() == 'Linux':
            libpython = glob(f'{sys.prefix}/lib/libpython*.so')[0]
        elif platform.system() == 'Windows':
            libpython = glob(f'{sys.prefix}/python*.dll')[0]
        else:
    ...
    if args.use_conda_shared_libs:
        if platform.system() != 'Windows':
            kernel_env['LD_LIBRARY_PATH'] += f':{sys.prefix}/lib'
        else:
...
def validate_kernel_env(kernel_env):
    ...
    if platform.system() == 'Windows':
        if not os.path.isfile(kernel_env['PYTHONPATH'] + '/lldb/_lldb.pyd'):
            raise Exception(f"lldb python libs not found at {kernel_env['PYTHONPATH']}")
    elif not os.path.isfile(kernel_env['PYTHONPATH'] + '/lldb/_lldb.so'):
        raise Exception(f"lldb python libs not found at {kernel_env['PYTHONPATH']}")
    if not os.path.isfile(kernel_env['REPL_SWIFT_PATH']):
        raise Exception(
            f"repl_swift binary not found at {kernel_env['REPL_SWIFT_PATH']}"
        )
    if 'SWIFT_BUILD_PATH' in kernel_env and \
            not os.path.isfile(kernel_env['SWIFT_BUILD_PATH']):
        raise Exception(
            f"swift-build binary not found at {kernel_env['SWIFT_BUILD_PATH']}"
        )
    if 'SWIFT_PACKAGE_PATH' in kernel_env and \
            not os.path.isfile(kernel_env['SWIFT_PACKAGE_PATH']):
        raise Exception(
            f"swift-package binary not found at {kernel_env['SWIFT_PACKAGE_PATH']}"
        )
    if 'PYTHON_LIBRARY' in kernel_env and \
            not os.path.isfile(kernel_env['PYTHON_LIBRARY']):
        raise Exception(f"python library not found at {kernel_env['PYTHON_LIBRARY']}")
    ...
    for index, lib_path in enumerate(lib_paths):
        ...
        if index == 0:
            raise Exception(f'swift libs not found at {lib_path}')
        # Other LD_LIBRARY_PATHs may be appended for other libs.
        raise Exception(f'shared lib dir not found at {lib_path}')
...
def main():
    ...
    print(f"Registered kernel \'{args.kernel_name}\' as \'{kernel_code_name}\'!")
